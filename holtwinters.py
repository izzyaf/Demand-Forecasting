...
def RMSE(params, *args):
    ...
    trend[-1] = (sum(dataframe.iloc[period_len:2 * period_len]) - sum(
        dataframe.iloc[0:period_len])) / period_len ** 2
    forecast = []
...
def multiplicative(input_dataframe, period_len, next_periods, alpha=None, beta=None, gamma=None):
    ...
    forecast = pd.Series(data=np.nan, index=[dataframe.index[period_len]])
    ...
    if alpha is None or beta is None or gamma is None:
        ...
        for i in range(period_len, t + next_periods):
            if i >= t:
                T = i - t
                forecast = forecast.append(pd.Series(data=(smooth[t - 1] + T * trend[t - 1]) * season[i - period_len],
                                                     index=[
                                                         forecast.index[forecast.count() - 1] + DateOffset(months=1)]))
                ...
                if i == period_len:
                    forecast[0] = (smooth[-1] + trend[-1]) * season[-period_len]
                else:
                    forecast = forecast.append(pd.Series(data=(smooth[-1] + trend[-1]) * season[-period_len],
                                                     index=[
                                                         forecast.index[forecast.count() - 1] + DateOffset(months=1)]))
